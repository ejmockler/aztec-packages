  ...
  implements WorldStateSynchronizer, L2BlockStreamLocalDataProvider, L2BlockStreamEventHandler
{
  ...
  private latestBlockNumberAtStart = BlockNumber(0);
  ...
  private async handleChainFinalized(blockNumber: BlockNumber) {
    ...
    const newHistoricBlock = BlockNumber.fromBigInt(
      BigInt(summary.finalizedBlockNumber) - BigInt(this.historyToKeep) + 1n,
    );
