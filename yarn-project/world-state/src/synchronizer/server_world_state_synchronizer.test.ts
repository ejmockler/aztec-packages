...
describe('ServerWorldStateSynchronizer', () => {
  ...
  beforeEach(() => {
    ...
    merkleTreeDb.getStatusSummary.mockResolvedValue({
      unfinalizedBlockNumber: BlockNumber.fromBigInt(BigInt(latestHandledBlockNumber)),
      finalizedBlockNumber: BlockNumber.fromBigInt(0n),
      oldestHistoricalBlock: BlockNumber.fromBigInt(0n),
  ...
  it('immediately syncs if no new blocks', async () => {
    blockAndMessagesSource.getBlockNumber.mockResolvedValue(BlockNumber(0));
...
class TestWorldStateSynchronizer extends ServerWorldStateSynchronizer {
  public latest = { number: BlockNumber(0), hash: '' };
  public finalized = { number: BlockNumber(0), hash: '' };
  public proven = { number: BlockNumber(0), hash: '' };
