  ...
  describe(testName, () => {
    ...
    describe('getBlocks', () => {
      ...
      it('throws an error if `from` it is out of range', async () => {
        await expect(store.getPublishedBlocks(BlockNumber(INITIAL_L2_BLOCK_NUM - 100), 1)).rejects.toThrow(
          'Invalid start: -99',
        );
      });
    ...
    describe('getPublicLogs', () => {
      ...
      it('intersecting works', async () => {
        let logs = (await store.getPublicLogs({ fromBlock: BlockNumber(-10), toBlock: BlockNumber(-5) })).logs;
        ...
        logs = (await store.getPublicLogs({ fromBlock: BlockNumber(-100), toBlock: +100 })).logs;
